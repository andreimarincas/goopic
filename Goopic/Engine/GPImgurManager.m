...
- (void)uploadImage:(UIImage *)image withName:(NSString *)name completion:(UploadCompletionBlock)completion
{
    ...
    [imgurSession POST:uploadURL parameters:nil constructingBodyWithBlock:appendImageBlock
             ...
             success:^(NSURLSessionDataTask *task, id responseObject) {
                 ...
                 if (completion)
                 {
                     ...
                     {
                         NSError *error = [NSError errorWithDomain:@"GPImgurManager" code:GPErrorLinkIsNil
                                                          userInfo:@{ NSLocalizedDescriptionKey : @"Imgur returned nil link after upload." }];
                         completion(@"", @"", error);
                     }
                 }
